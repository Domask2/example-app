...
class MagicService
{
    ...
    static function index($db_key, $ds_key)
    {
        ...
        if ($ds['type'] === 'FUNCTION') {
            ...
            try {
                ...
                $original = DB::connection('udb')->select('select * from ' . $ds['key'] . '()');
            ...
            if ($curPage !== 0) {
                ...
                if ($orderBy)
                    $original = $magicEntity->limit($perPage)->offset($offset)->orderBy($orderBy)->get();
                else
                    $original = $magicEntity->limit($perPage)->offset($offset)->get();
            } else {
                $count = $magicEntity->limit(1000)->count();
                if ($orderBy)
                    $original = $magicEntity->limit(1000)->orderBy($orderBy)->get();
                else
     ...
     * @return bool
     */
    public function execute($db_key, $ds_key): bool
    {
        $user = auth() ? auth()->user() : '';
        $dbRepository = new DataBaseRepository();
        $db = $dbRepository->findByKey($db_key);
